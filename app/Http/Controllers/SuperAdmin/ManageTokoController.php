...
class ManageTokoController extends Controller
{
    ...
    public function list_toko(Request $request)
    {
        ...
        $get_toko = Toko::join('users', 'toko.id_toko', 'users.id_toko')
                            ->where('users.status_active', $status)
                            ->where('users.is_toko', 1)
                            ->selectRaw("toko.id_toko, id_subscription, nama_toko, alamat_toko, email, username, no_telepon, CONCAT('" . $media_url . "', url_logo) as url_logo, users.created_at");

        if (!empty($search)) {
            $get_toko->where(function ($q) use ($search) {
                return $q->where('toko.nama_toko', 'like', '%' . $search . '%');
            });
        }

        $get_toko = $get_toko->paginate($per_page)->withQueryString();

        if($get_toko){
            $result['status'] = true;
            $result['message'] = 'Data Berhasil Didapatkan.';
            $result['data'] = $get_toko;
        } else {
            $result['status'] = false;
